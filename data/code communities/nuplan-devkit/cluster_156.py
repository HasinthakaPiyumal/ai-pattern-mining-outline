...
class OverviewTab(BaseTab):
    ...
    def __init__(self, doc: Document, experiment_file_data: ExperimentFileData):
        ...
        self._default_columns = [TableColumn(**OverviewTabExperimentTableColumnConfig.get_config()), TableColumn(**OverviewTabScenarioTypeTableColumnConfig.get_config()), TableColumn(**OverviewTabPlannerTableColumnConfig.get_config())]
        self.table = DataTable(source=self._default_datasource, columns=self._default_columns, **OverviewTabDataTableConfig.get_config())
        ...
        for index, metric_aggregator_dataframes in enumerate(self.experiment_file_data.metric_aggregator_dataframes):
            ...
            for metric_aggregator_filename, metric_aggregator_dataframe in metric_aggregator_dataframes.items():
                for _, row_data in metric_aggregator_dataframe.iterrows():
                    ...
                    data[metric_aggregator_filename].append(OverviewAggregatorData(aggregator_type=aggregator_type, planner_name=planner_name, scenario_type=scenario_type, num_scenarios=int(num_scenarios), score=metric_score, aggregator_file_name=metric_aggregator_filename))
    ...
    def _update_overview_table(self, data: Dict[str, List[OverviewAggregatorData]]) -> None:
        ...
        """
        planner_names = sorted(list({metric_aggregator_data.planner_name for _, metric_aggregator_data_list in data.items() for metric_aggregator_data in metric_aggregator_data_list if metric_aggregator_data.planner_name in self.enable_planner_names}))
        ...
        for metric_file, metric_aggregator_data_list in data.items():
            ...
            for metric_aggregator_data in metric_aggregator_data_list:
                ...
                if metric_aggregator_data.planner_name not in self.enable_planner_names:
                    continue
                metric_scores[metric_aggregator_data.planner_name][metric_aggregator_data.scenario_type] = [np.round(metric_aggregator_data.score, 4), metric_aggregator_data.num_scenarios]
            initial_planner_names = list(set(initial_planner_names))
            if metric_scores:
                metric_aggregator_files += [metric_file] + [''] * (len(metric_aggregator_data_list) // len(initial_planner_names) - 1)
            metric_aggregator_file_scenario_types: List[str] = []
            for planner_name, values in metric_scores.items():
                sorted_metric_scores: Dict[str, List[float]] = dict(sorted(list(values.items()), key=lambda item: item[0]))
                sorted_final_metric_scores = {f'all ({sorted_metric_scores['final_score'][1]})': sorted_metric_scores['final_score'][0]}
                sorted_final_metric_scores.update({f'{scenario_type} ({score[1]})': score[0] for scenario_type, score in sorted_metric_scores.items() if scenario_type != 'final_score'})
        ...
        if planner_name_columns:
            data_sources: Dict[str, List[Any]] = {'experiment': metric_aggregator_files, 'scenario_type': scenario_types}
            data_sources.update(planner_name_columns)
            planner_table_columns = [TableColumn(field=planner_name, title=f'Evaluation Score {index + 1}: {planner_name}', sortable=False) for index, planner_name in enumerate(planner_name_columns.keys())]
            columns = [TableColumn(**OverviewTabExperimentTableColumnConfig.get_config()), TableColumn(**OverviewTabScenarioTypeTableColumnConfig.get_config())]
