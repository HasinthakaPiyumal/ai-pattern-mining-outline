...
class TestTypes(unittest.TestCase):
    ...
    def test_tgraph(self):
        ...
        pydantic_graph = TGraph.Model(entities=[TEntity.Model(name='Entity1', type='Type1', desc='Description1')], relationships=[TRelation.Model(source='Entity1', target='Entity2', desc='Relation description')], other_relationships=[])
    ...
    def test_tcontext(self):
        entities = [TEntity(name='Entity1', type='Type1', description='Sample description 1')] * 8 + [TEntity(name='Entity2', type='Type2', description='Sample description 2')] * 8
        relationships = [TRelation(source='Entity1', target='Entity2', description='Relation description 12')] * 8 + [TRelation(source='Entity2', target='Entity1', description='Relation description 21')] * 8
        chunks = [TChunk(id=i, content=f'Long and repeated chunk content {i}' * 4, metadata={'key': f'value {i}'}) for i in range(16)]
        for r, c in zip(relationships, chunks):
            r.chunks = [c.id]
        context = TContext(entities=[(e, TScore(0.9)) for e in entities], relations=[(r, TScore(0.8)) for r in relationships], chunks=[(c, TScore(0.7)) for c in chunks])
        max_chars = {'entities': 128, 'relations': 128, 'chunks': 512}
        csv = context.truncate(max_chars.copy(), True)
        csv_entities = re.findall('## Entities\\n```csv\\n(.*?)\\n```', csv, re.DOTALL)
        csv_relationships = re.findall('## Relationships\\n```csv\\n(.*?)\\n```', csv, re.DOTALL)
        csv_chunks = re.findall('## Sources\\n.*=====', csv, re.DOTALL)
        self.assertEqual(len(csv_entities), 1)
        self.assertEqual(len(csv_relationships), 1)
        self.assertEqual(len(csv_chunks), 1)
        self.assertGreaterEqual(sum(max_chars.values()) + 16, len(csv_entities[0]) + len(csv_relationships[0]) + len(csv_chunks[0]))

    def test_tqueryresponse(self):
        context = TContext(entities=[('Entity1', TScore(0.9))], relations=[('Relation1', TScore(0.8))], chunks=[('Chunk1', TScore(0.7))])
