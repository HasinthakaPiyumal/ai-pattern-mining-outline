...
class SendMessage(Tool):
    ...
    def __call__(self, message: str, recipient: Optional[str]=None, metadata: Optional[Dict[str, Any]]=None) -> bool:
        """
        ...
        if not hasattr(self.namespace, 'a2a_handler') or not self.namespace.a2a_handler:
            ...
            return False
        try:
            a2a_message = Message(messageId=str(uuid.uuid4()), role='agent', parts=[Part(root=TextPart(text=message))], metadata={'sender': self.namespace.agent_id, 'message_type': 'text', 'timestamp': time.time(), 'recipient': recipient})
