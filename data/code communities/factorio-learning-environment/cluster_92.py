...
def test_script_caching():
    ...
    for i in range(RUNS):
        ...
        instance = FactorioInstance(address='localhost', bounding_box=200, tcp_port=27000, inventory=INVENTORY, cache_scripts=True)
    ...
    for i in range(RUNS):
        start_time = time.time()
        instance = FactorioInstance(address='localhost', bounding_box=200, tcp_port=27000, inventory=INVENTORY, cache_scripts=False)
        end_time = time.time() - start_time
