...
class APIFactory:
    PROVIDERS = {'open-router': {'base_url': 'https://openrouter.ai/api/v1', 'api_key_env': 'OPEN_ROUTER_API_KEY', 'key_manager_provider': 'open-router', 'model_transform': lambda m: m.replace('open-router-', '') if m.startswith('open-router-') else m}, 'claude': {'base_url': 'https://api.anthropic.com/v1', 'api_key_env': 'ANTHROPIC_API_KEY', 'key_manager_provider': 'anthropic'}, 'deepseek': {'base_url': 'https://api.deepseek.com', 'api_key_env': 'DEEPSEEK_API_KEY', 'key_manager_provider': 'deepseek'}, 'gemini': {'base_url': 'https://generativelanguage.googleapis.com/v1beta/openai/', 'api_key_env': 'GEMINI_API_KEY', 'key_manager_provider': 'gemini'}, 'together': {'base_url': 'https://api.together.xyz/v1', 'api_key_env': 'TOGETHER_API_KEY', 'key_manager_provider': 'together'}, 'openai': {'base_url': 'https://api.openai.com/v1', 'api_key_env': 'OPENAI_API_KEY', 'key_manager_provider': 'openai'}}
    ...
    def __init__(self, model: str, beam: int=1, api_key_config_file: str=None):
        ...
        self.model = model
        self.beam = beam
        self.api_key_config_file = api_key_config_file
    ...
    def _get_api_key(self, provider_config: dict) -> str:
        ...
        if 'key_manager_provider' in provider_config:
            ...
            if not self.api_key_manager:
                try:
                    config_file = self.api_key_config_file or os.getenv('FLE_API_KEY_CONFIG_FILE') or os.getenv('API_KEY_CONFIG_FILE')
                    if config_file:
                        ...
                        if get_api_key_manager_func:
                            ...
                            logging.info(f'Reinitialized API key manager in child process: {config_file}')
                except Exception as e:
                    logging.warning(f'Failed to reinitialize API key manager in child process: {e}')
        ...
        if env_key:
            ...
            return env_key
        raise ValueError(f'No API key available for provider. Set {env_var} or configure key manager.')

    def _mark_key_result(self, provider_config: dict, api_key: str, success: bool, error: Exception=None):
        ...
        if self.api_key_manager and 'key_manager_provider' in provider_config:
            ...
            if success:
                ...
                self.api_key_manager.mark_key_error(key_manager_provider, api_key, error)
    ...
    async def acall(self, **kwargs):
        ...
        client = AsyncOpenAI(base_url=provider_config['base_url'], api_key=api_key, max_retries=0)
        try:
            api_params = {'model': model_to_use, 'messages': messages, 'max_tokens': kwargs.get('max_tokens', 256), 'temperature': kwargs.get('temperature', 0.3), 'logit_bias': kwargs.get('logit_bias'), 'n': kwargs.get('n_samples'), 'stop': kwargs.get('stop_sequences'), 'presence_penalty': kwargs.get('presence_penalty'), 'frequency_penalty': kwargs.get('frequency_penalty'), 'stream': False}
            ...
            if hasattr(response, 'usage') and hasattr(response.usage, 'reasoning_tokens'):
                async with timing_tracker.track_async('reasoning', model=model_to_use, tokens=response.usage.reasoning_tokens):
                    pass
            return response
        except Exception as e:
