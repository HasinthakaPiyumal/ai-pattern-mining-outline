...
class GymTrajectoryRunner:
    ...
    def __init__(self, config: GymEvalConfig, gym_env: FactorioGymEnv, process_id: int, db_client: Optional[DBClient], log_dir: Optional[str]=None, reset_states: bool=False, wandb_logger: Optional['WandBLogger']=None):
        self.config = config
        self.agents = config.agents
        self.gym_env = gym_env
        self.instance = gym_env.unwrapped.instance
        self.db_client = db_client
        self.process_id = process_id
        self.start_time = time.time()
        self.reset_states = reset_states
        self.wandb_logger = wandb_logger
        self.logger = TrajectoryLogger(start_time=self.start_time, trajectory_length=self.config.task.trajectory_length, log_dir=log_dir)

    def _log_trajectory_state(self, iteration_start: float, agent: GymAgent, agent_idx: int, agent_step: int, production_score: float, program: Program, observation: Observation):
        ...
        if self.wandb_logger and WANDB_ANALYSIS_AVAILABLE:
            try:
                ...
                self.wandb_logger.log_trajectory_progress(version=self.config.version, instance=self.process_id, step=agent_step, reward=program.value, production_score=production_score, model=agent.model, task=task_name, elapsed_time=elapsed_time, tokens_used=program.token_usage)
            except Exception as e:
                print(f'Warning: Failed to log to WandB: {e}')

    async def create_program_from_policy(self, policy, agent_idx: int, reward: float, response: str, error_occurred: bool, achievements: Dict[str, Any], game_state: GameState, production_score: float) -> Program:
        ...
        program = Program(code=policy.code, conversation=policy.input_conversation, response=response, token_usage=policy.meta.total_tokens, completion_token_usage=policy.meta.output_tokens, prompt_token_usage=policy.meta.input_tokens, version=self.config.version, instance=agent_idx, model=self.agents[agent_idx].model, version_description=self.config.version_description, value=reward, state=game_state, achievements=achievements, meta={'model': self.agents[agent_idx].model, 'process_id': self.process_id, 'error_occurred': error_occurred, 'sweep_id': os.getenv('FLE_SWEEP_ID', 'unknown'), 'production_score': production_score}, depth=depth)
    ...
    async def _initialize_trajectory_state(self) -> Tuple[GameState, List[int]]:
        ...
        if self.config.version and self.db_client is not None:
            for agent_idx in range(len(self.agents)):
                current_state, agent_conversation, parent_id, depth = await self.db_client.get_resume_state(resume_version=self.config.version, process_id=self.process_id, agent_idx=agent_idx)
                if current_state:
            ...
            try:
                while not agent_completed and agent_steps[agent_idx] < max_steps:
                    ...
                    if not policy:
                        ...
                        break
                    action = Action(code=policy.code, agent_idx=agent_idx, game_state=current_state if self.reset_states else None)
                    ...
                    program = await self.create_program_from_policy(policy=policy, agent_idx=agent_idx, reward=reward, response=obs_dict['raw_text'], error_occurred=info['error_occurred'], achievements=info['achievements'], game_state=output_game_state, production_score=production_score)
                    await agent.update_conversation(observation, previous_program=program)
                    self._log_trajectory_state(iteration_start, agent, agent_idx, agent_steps[agent_idx], production_score, program, observation)
                    if self.reset_states:
                        agent_completed, update_state = agent.check_step_completion(observation)
                        if update_state:
                            current_state = output_game_state
                    if done:
                        completion_result = CompletionResult(step=agent_steps[agent_idx], reason=CompletionReason.SUCCESS)
