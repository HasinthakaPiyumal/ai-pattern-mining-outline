...
def process_node(node: Node, history: List[str]):
    ...
    for i, child in enumerate(node.children[:-1]):
        sibling_history = history + [f'{node.number}. {node.content}'] + [f'   {node.children[j].number}. {node.children[j].content}' for j in range(i + 1)]
    ...
    if node.children:
        ...
        leaf_history = history + [f'{node.number}. {node.content}', f'   {last_child.number}. {last_child.content}', '   <>']
    ...
    with open(output_file, 'a') as fo:
        for file in processed_dir.glob('*.xml'):
            ...
            for input_text, output_text in examples:
                fo.write(json.dumps({'messages': [{'role': 'system', 'content': 'You are a helpful assistant that decides on the most appropriate Factorio game objective'}, {'role': 'user', 'content': input_text}, {'role': 'assistant', 'content': output_text}]}) + '\n')
