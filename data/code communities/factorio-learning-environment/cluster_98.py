...
def test_place_pipe_next_to_offshore_pump(game):
    ...
    for direction in [Direction.RIGHT, Direction.DOWN, Direction.UP]:
        for spacing in range(3):
            ...
            expected_position = calculate_expected_position(ref_entity.position, direction, spacing, ref_entity, placed_entity)
            assert placed_entity.position.is_close(expected_position, tolerance=1), f'Misplacement: {ref_proto.value[0]} -> {placed_proto.value[0]}, Direction: {direction}, Spacing: {spacing}, Expected: {expected_position}, Got: {placed_entity.position}'
...
def test_place_drill_and_furnace_next_to_iron_ore(game):
    ...
    furnace = game.place_entity_next_to(Prototype.StoneFurnace, reference_position=entity.position, direction=Direction.DOWN)
...
def test_place_entity_next_to(game, entity_prototype, surrounding_entity_prototype):
    for spacing in range(0, 3):
        ...
        for direction in directions:
            surrounding_entity = game.place_entity_next_to(surrounding_entity_prototype, reference_position=entity.position, direction=direction, spacing=spacing)
            ...
            expected_position = calculate_expected_position(entity.position, direction, spacing, entity, surrounding_entity)
            print(f'Expected position: {expected_position}')
            ...
            try:
                assert abs(x_diff) <= tolerance and abs(y_diff) <= tolerance, f'Entity not in expected position for direction {direction} with spacing {spacing}. Expected {expected_position}, got {surrounding_entity.position}. Difference: x={x_diff}, y={y_diff}'
    ...
    belt = game.place_entity_next_to(Prototype.TransportBelt, reference_position=boiler.position, direction=Direction.RIGHT, spacing=0)
    ...
    print(f'Difference: x={x_diff}, y={y_diff}')
    assert abs(x_diff) <= tolerance and abs(y_diff) <= tolerance, f'Transport belt not in expected position. Expected {expected_belt_position}, got {belt.position}. Difference: x={x_diff}, y={y_diff}'
