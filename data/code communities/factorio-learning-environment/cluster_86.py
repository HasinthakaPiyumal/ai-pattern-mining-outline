...
def create_solid_input_line(game, item_prototype: Prototype, start_position: Position, building: Entity, y_offset=0):
    ...
    chest_inserter = game.place_entity(Prototype.BurnerInserter, Direction.LEFT, Position(x=start_position.x - 1, y=start_position.y))
    ...
    game.connect_entities(chest_inserter, target_inserter, connection_type=Prototype.TransportBelt)
    ...
    for recipe in recipes_to_test:
        ...
        for ingredient in requirements.ingredients:
            if ingredient.type == 'fluid':
                tank, position = setup_fluid_input(game, ingredient.name, Position(x=building_position.x + current_x, y=building_position.y - 20))
        ...
        for ingredient in requirements.ingredients:
            if ingredient.type != 'fluid':
                ...
                inserter = create_solid_input_line(game, prototype_by_name[ingredient.name], chest_position, building, y_offset=current_y)
        ...
        for product in requirements.products:
            if product.type == 'fluid':
                ...
                tank = game.place_entity(Prototype.StorageTank, position=Position(x=building_position.x + current_x, y=building_position.y + 16), direction=Direction.UP)
                output_tanks[product.name] = tank.position
                current_x += 8
            else:
                game.move_to(building.position)
                output_inserter = game.place_entity_next_to(Prototype.BurnerInserter, reference_position=building.position, direction=Direction.LEFT)
