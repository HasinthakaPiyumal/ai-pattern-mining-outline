...
class ComposeGenerator:
    ...
    def __init__(self, attach_mod=False, save_file=None, scenario='default_lab_scenario', state_dir: Path | None=None, work_dir: Path | None=None, pkg_scenarios_dir: Path | None=None, pkg_config_dir: Path | None=None):
    ...
    def _command(self):
        ...
        args = [f'--port {self.internal_game_port}', f'--rcon-port {self.internal_rcon_port}', f'--rcon-password {self.rcon_password}', '--server-settings /opt/factorio/config/server-settings.json', '--map-gen-settings /opt/factorio/config/map-gen-settings.json', '--map-settings /opt/factorio/config/map-settings.json', '--server-adminlist /opt/factorio/config/server-adminlist.json', '--server-banlist /opt/factorio/config/server-banlist.json', '--server-whitelist /opt/factorio/config/server-whitelist.json', '--use-server-whitelist']
    ...
    def _copy_save(self, save_file: str):
        ...
        with zipfile.ZipFile(save_file, 'r') as zf:
            if 'level.dat' not in zf.namelist():
                raise ValueError(f"Save file '{save_file}' does not contain a 'level.dat' file.")
        shutil.copy2(save_file, save_dir / save_file_name)
        print(f'Copied save file to {save_dir / save_file_name}')

    def _mods_volume(self):
        return {'source': str(self._mod_path().resolve()), 'target': '/opt/factorio/mods', 'type': 'bind'}

    def _save_volume(self):
        return {'source': str(self._save_path().resolve()), 'target': '/opt/factorio/saves', 'type': 'bind'}

    def _screenshots_volume(self):
        screenshots_dir = self.work_dir / '.fle' / 'data' / '_screenshots'
        screenshots_dir.mkdir(parents=True, exist_ok=True)
        return {'source': str(screenshots_dir.resolve()), 'target': '/opt/factorio/script-output', 'type': 'bind'}
    ...
    def _scenarios_volume(self):
        ...
        return {'source': str(scenarios_dir.resolve()), 'target': '/opt/factorio/scenarios', 'type': 'bind'}
    ...
    def _config_volume(self):
        ...
        return {'source': str(config_dir.resolve()), 'target': '/opt/factorio/config', 'type': 'bind'}
    ...
    def services_dict(self, num_instances):
        ...
        for i in range(num_instances):
            ...
            volumes = [self._scenarios_volume(), self._config_volume(), self._screenshots_volume()]
            if self.save_file:
                volumes.append(self._save_volume())
            if self.attach_mod:
                volumes.append(self._mods_volume())
            services[f'factorio_{i}'] = {'image': self.image, 'platform': self._docker_platform(), 'command': self._command(), 'deploy': {'resources': {'limits': {'cpus': '1', 'memory': '1024m'}}}, 'entrypoint': [], 'ports': [f'{host_game}:{self.internal_game_port}/udp', f'{host_rcon}:{self.internal_rcon_port}/tcp'], 'pull_policy': 'missing', 'restart': 'unless-stopped', 'user': 'factorio', 'volumes': volumes}
        return services
...
class ClusterManager:
    ...
    def generate(self, num_instances, scenario, attach_mod=False, save_file=None):
        generator = ComposeGenerator(attach_mod=attach_mod, save_file=save_file, scenario=scenario, state_dir=self.state_dir, work_dir=self.work_dir, pkg_scenarios_dir=self.pkg_scenarios_dir, pkg_config_dir=self.pkg_config_dir)
        generator.write(str(self.compose_path), num_instances)
        print(f'Generated compose at {self.compose_path} for {num_instances} instance(s) using scenario {scenario}')
    ...
    def start(self, num_instances, scenario, attach_mod=False, save_file=None):
        ...
        if listening:
            ...
            print("It looks like a Factorio cluster (or another service) is running. Stop it with 'fle cluster stop' (or 'docker compose -f docker-compose.yml down' in fle/cluster) and retry.")
            sys.exit(1)
        ...
        print(f'Factorio cluster started with {num_instances} instance(s) using scenario {scenario}')

    def stop(self):
        if not self.compose_path.exists():
            print('Error: docker-compose.yml not found in state dir. No cluster to stop.')
            sys.exit(1)
    ...
    def restart(self):
        if not self.compose_path.exists():
            print('Error: docker-compose.yml not found in state dir. No cluster to restart.')
            sys.exit(1)
        print('Restarting existing Factorio services without regenerating docker-compose...')
        self._run_compose(['-f', str(self.compose_path), 'restart'])
        print('Factorio services restarted.')

    def logs(self, service: str='factorio_0'):
        if not self.compose_path.exists():
            print('Error: docker-compose.yml not found in state dir. Nothing to show logs for.')
            sys.exit(1)
        self._run_compose(['-f', str(self.compose_path), 'logs', service])

    def show(self):
        ps_cmd = ['docker', 'ps', '--filter', 'name=factorio_', '--format', 'table {{.Names}}\t{{.Ports}}']
...
def setup_compose_cmd():
    candidates = [['docker', 'compose', 'version'], ['docker-compose', '--version']]
