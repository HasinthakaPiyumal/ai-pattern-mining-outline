...
def yt_vid_url_to_mp4(yt_vid_url, mp4_dir_save_path):
    ...
    ydl_opts = {'format': 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best', 'outtmpl': os.path.join(mp4_dir_save_path, '%(title)s.%(ext)s'), 'restrictfilenames': True}
...
def yt_vid_id_to_srt(transcript, yt_video_id, srt_save_path):
    ...
    for i, entry in enumerate(transcript):
        ...
        srt_content.append(f'{int(start_hours):02}:{int(start_minutes):02}:{int(start_seconds):02},{start_milliseconds:03} --> {int(end_hours):02}:{int(end_minutes):02}:{int(end_seconds):02},{end_milliseconds:03}')
