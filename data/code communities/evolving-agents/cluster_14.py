...
def simulate_search_results(query, task_context=None):
    ...
    results = []
    content_scores = {'OAuth2Client': 0.92, 'AuthTestSuite': 0.85, 'APIDocGenerator': 0.78, 'DatabaseConnector': 0.45}
    task_adjustments = {'implementation': {'OAuth2Client': 0.3, 'AuthTestSuite': -0.1, 'APIDocGenerator': -0.15, 'DatabaseConnector': 0.05}, 'testing': {'OAuth2Client': -0.05, 'AuthTestSuite': 0.35, 'APIDocGenerator': -0.1, 'DatabaseConnector': -0.15}, 'documentation': {'OAuth2Client': -0.1, 'AuthTestSuite': -0.15, 'APIDocGenerator': 0.4, 'DatabaseConnector': -0.2}}
    task_adj = None
    if task_context:
    ...
    for component in SAMPLE_COMPONENTS:
        ...
        results.append({'name': name, 'final_score': final_score, 'content_score': content_score, 'task_score': task_score, 'record_type': component['record_type'], 'description': component['description'][:100]})
...
class SmartLibrary:
    ...
    def __init__(self, llm_service: Optional[LLMService]=None, container: Optional['DependencyContainer']=None, mongodb_uri: Optional[str]=None, mongodb_db_name: Optional[str]=None, components_collection_name: str='eat_components', mongodb_client: Optional[MongoDBClient]=None):
        ...
        self.atlas_vs_idx_applicability_embedding = 'idx_components_applicability_embedding'
        ...
        if container and container.has('mongodb_client'):
        ...
        elif isinstance(llm_service, MongoDBClient):
            ...
            if container:
                container.register('llm_service', self.llm_service)
        elif isinstance(mongodb_uri, MongoDBClient):
    ...
    async def _ensure_indexes(self):
        ...
        try:
            ...
            try:
                await self.components_collection.create_index([('name', pymongo.TEXT), ('description', pymongo.TEXT), ('tags', pymongo.TEXT), ('domain', pymongo.TEXT)], name='component_text_search_fallback_idx', default_language='english', weights={'name': 5, 'tags': 3, 'description': 2, 'domain': 1}, background=True)
    ...
    def _get_record_vector_text(self, record: Dict[str, Any]) -> str:
        ...
        interface_info = ''
        if record_type == 'TOOL':
            signatures = re.findall('def\\s+(\\w+\\([^)]*\\))', code_snippet)
            if signatures:
                interface_info += 'Functions: ' + ', '.join(signatures) + '. '
            api_endpoints = re.findall('@\\w+\\.(?:route|get|post|put|delete)\\([\\\'"]([^\\\'"]+)[\\\'"]', code_snippet)
            if api_endpoints:
                interface_info += 'API endpoints: ' + ', '.join(api_endpoints) + '. '
            class_defs = re.findall('class\\s+(\\w+)(?:\\([^)]*\\))?:', code_snippet)
            if class_defs:
                interface_info += 'Classes: ' + ', '.join(class_defs) + '. '
        return f'Component Name: {name}. Type: {record_type}. Domain: {domain}. Functional Purpose: {description}. {interface_info}{('Relevant Tags: ' + tags + '. ' if tags else '')}{('Usage Statistics: ' + usage_stats + '. ' if usage_stats else '')}Core Implementation Snippet (summary):\n```\n{code_snippet[:1000]}\n```'
    ...
    async def generate_applicability_text(self, record_data: Dict[str, Any]) -> str:
        ...
        prompt = prompt_template.format(name=name, record_type=record_type, domain=domain, description=description, code_summary=code_summary)
    ...
    async def save_record(self, record: Dict[str, Any]) -> str:
        ...
        if not record_id:
            raise ValueError("Record must have an 'id' to be saved.")
        ...
        try:
                ...
                if dt_field in record and isinstance(record[dt_field], str):
                    try:
                        ...
                        if not isinstance(record[dt_field], datetime):
                            ...
                            record[dt_field] = datetime.now(timezone.utc)
            ...
            logger.error(f'Error saving record {record_id} to MongoDB: {e}', exc_info=True)
            raise
    ...
    async def semantic_search(self, query: str, task_context: Optional[str]=None, record_type: Optional[str]=None, domain: Optional[str]=None, limit: int=5, threshold: float=0.0, task_weight: Optional[float]=0.7) -> List[Tuple[Dict[str, Any], float, float, float]]:
        if self.components_collection is None:
        ...
        if not vector_search_enabled:
            ...
            return await self._perform_fallback_search(query, query_embedding_orig, task_embedding_for_fallback, record_type, domain, limit, effective_task_weight, task_context)
        ...
        vs_stage_definition: Dict[str, Any] = {'index': current_atlas_search_index_name, 'path': vector_path_for_search, 'queryVector': query_vector_for_search, 'numCandidates': limit * 20, 'limit': limit * 10}
        mql_filter_conditions = []
        if record_type:
            mql_filter_conditions.append({'record_type': record_type})
        if domain:
            mql_filter_conditions.append({'domain': domain})
        ...
        projected_doc_fields = {'_id': 0, 'id': 1, 'name': 1, 'record_type': 1, 'domain': 1, 'description': 1, 'version': 1, 'usage_count': 1, 'success_count': 1, 'tags': 1, 'metadata': 1, 'content_embedding': 1, 'applicability_embedding': 1}
        search_pipeline: List[Dict[str, Any]] = [{'$vectorSearch': vs_stage_definition}, {'$project': {**projected_doc_fields, 'similarity_score': {'$meta': 'vectorSearchScore'}}}]
        ...
        for doc in candidate_docs:
            ...
            if task_context and task_embedding_for_fallback:
                ...
                if query_embedding_orig and content_embedding_from_doc:
                    ...
                    content_score = 0.0 if content_embedding_from_doc else 0.5
            else:
                ...
                if query_embedding_orig and applicability_embedding_from_doc:
                    ...
                    task_score = 0.0 if applicability_embedding_from_doc else 0.5
            ...
            if adjusted_score >= threshold:
                ...
                doc_copy.pop('similarity_score', None)
    ...
    async def _try_text_search(self, query_text: str, record_type: Optional[str]=None, domain: Optional[str]=None, limit: int=5) -> List[Dict[str, Any]]:
        ...
        if record_type:
            text_search_filter['record_type'] = record_type
        if domain:
            text_search_filter['domain'] = domain
        try:
            projection_with_score = {'_id': 0, 'id': 1, 'name': 1, 'record_type': 1, 'domain': 1, 'description': 1, 'version': 1, 'tags': 1, 'metadata': 1, 'usage_count': 1, 'success_count': 1, 'content_embedding': 1, 'applicability_embedding': 1, 'text_search_score': {'$meta': 'textScore'}}
            cursor = self.components_collection.find(text_search_filter, projection_with_score).sort([('text_search_score', {'$meta': 'textScore'})]).limit(limit)
    ...
    async def _perform_fallback_search(self, query: str, query_embedding_orig: List[float], task_embedding: Optional[List[float]], record_type: Optional[str]=None, domain: Optional[str]=None, limit: int=5, effective_task_weight: float=0.7, task_context: Optional[str]=None) -> List[Tuple[Dict[str, Any], float, float, float]]:
        if self.components_collection is None:
            return []
        ...
        if not docs_to_score:
            ...
            if record_type:
                fallback_query_filter['record_type'] = record_type
            if domain:
                fallback_query_filter['domain'] = domain
            query_terms = [term.lower() for term in re.split('\\s+', query) if len(term) > 2]
            if query_terms:
                regex_conditions = []
            ...
            projection = {'_id': 0, 'id': 1, 'name': 1, 'record_type': 1, 'domain': 1, 'description': 1, 'version': 1, 'tags': 1, 'metadata': 1, 'usage_count': 1, 'success_count': 1, 'content_embedding': 1, 'applicability_embedding': 1}
            try:
    ...
    async def create_record(self, name: str, record_type: str, domain: str, description: str, code_snippet: str, version: str='1.0.0', status: str='active', tags: Optional[List[str]]=None, metadata: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        if self.components_collection is None:
        ...
        record = {'id': record_id, 'name': name, 'record_type': record_type, 'domain': domain, 'description': description, 'code_snippet': code_snippet, 'version': version, 'usage_count': 0, 'success_count': 0, 'fail_count': 0, 'status': status, 'created_at': current_time_dt, 'last_updated': current_time_dt, 'tags': tags or [], 'metadata': {**(metadata or {}), 'applicability_text': applicability_text}, 'content_embedding': content_embedding, 'applicability_embedding': applicability_embedding}
    ...
    async def update_usage_metrics(self, record_id: str, success: bool=True) -> None:
        if self.components_collection is None:
            ...
            return
        update_result = await self.components_collection.update_one({'id': record_id}, {'$inc': {'usage_count': 1, 'success_count': 1 if success else 0, 'fail_count': 0 if success else 1}, '$set': {'last_updated': datetime.now(timezone.utc)}})
        if update_result.matched_count == 0:
            logger.warning(f'Update metrics: record {record_id} not found.')
        else:
            logger.info(f'Updated usage metrics for record {record_id} (success={success}).')
    ...
    async def evolve_record(self, parent_id: str, new_code_snippet: str, description: Optional[str]=None, new_version: Optional[str]=None, status: str='active') -> Dict[str, Any]:
        if self.components_collection is None:
        ...
        if not parent:
            raise ValueError(f'Parent record {parent_id} not found')
        evolved_version_str = new_version or self._increment_version(parent['version'])
        current_time_dt = datetime.now(timezone.utc)
        evolved_desc = description or parent['description']
        traz_input = {'name': parent['name'], 'record_type': parent['record_type'], 'domain': parent['domain'], 'description': evolved_desc, 'code_snippet': new_code_snippet}
        ...
        evolved_metadata.update({'applicability_text': new_applicability_text, 'evolved_at': current_time_dt.isoformat(), 'evolved_from': parent_id, 'previous_version': parent['version']})
        evolved_record = {'id': str(uuid.uuid4()), 'name': parent['name'], 'record_type': parent['record_type'], 'domain': parent['domain'], 'description': evolved_desc, 'code_snippet': new_code_snippet, 'version': evolved_version_str, 'usage_count': 0, 'success_count': 0, 'fail_count': 0, 'status': status, 'created_at': current_time_dt, 'last_updated': current_time_dt, 'parent_id': parent_id, 'tags': parent.get('tags', []).copy(), 'metadata': evolved_metadata, 'content_embedding': new_content_embedding, 'applicability_embedding': new_applicability_embedding}
    ...
    async def deploy_component_version(self, component_id: str, deployed_status: str='active', parent_archived_status: str='archived') -> bool:
        """
    ...
    def _increment_version(self, version: str) -> str:
        ...
        try:
            ...
            logger.warning(f"Invalid version format '{version}', appending '.1'")
            return f'{version}.1'
    ...
    async def bulk_import(self, records_data: List[Dict[str, Any]]) -> int:
        ...
        for r_dict in records_data:
            ...
            temp_rec_data = {'name': r_dict.get('name', 'Unnamed Component'), 'record_type': r_dict.get('record_type', 'UNKNOWN'), 'domain': r_dict.get('domain', 'general'), 'description': r_dict.get('description', ''), 'code_snippet': r_dict.get('code_snippet', ''), 'tags': r_dict.get('tags', []), 'usage_count': r_dict.get('usage_count', 0), 'success_count': r_dict.get('success_count', 0)}
            ...
            last_updated = now_dt
            full_rec = {**r_dict, 'id': record_id, 'created_at': created_at, 'last_updated': last_updated, 'status': r_dict.get('status', 'active'), 'metadata': {**r_dict.get('metadata', {}), 'applicability_text': applicability_text}, 'content_embedding': content_embedding, 'applicability_embedding': applicability_embedding}
            for key, default_val in [('usage_count', 0), ('success_count', 0), ('fail_count', 0), ('tags', [])]:
                if key not in full_rec:
                    full_rec[key] = default_val
        ...
        try:
            ...
            return bwe.details.get('nInserted', 0) + bwe.details.get('nUpserted', 0) + bwe.details.get('nModified', 0)
        except Exception as e:
    ...
    async def get_status_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_candidates_cursor = self.components_collection.find({'status': 'active', 'usage_count': {'$gte': 5}}, projection).limit(50)
            ...
            return {'total_records': total_records, 'by_type': {'AGENT': type_counts.get('AGENT', 0), 'TOOL': type_counts.get('TOOL', 0), 'FIRMWARE': type_counts.get('FIRMWARE', 0), **{k: v for k, v in type_counts.items() if k not in ['AGENT', 'TOOL', 'FIRMWARE'] and k}}, 'by_status': status_counts, 'domains': distinct_domains, 'most_used': [{'id': r.get('id'), 'name': r.get('name'), 'usage_count': r.get('usage_count', 0)} for r in most_used], 'most_successful': [{'id': r.get('id'), 'name': r.get('name'), 'success_rate': r.get('success_rate', 0.0)} for r in most_successful], 'recently_updated': [{'id': r.get('id'), 'name': r.get('name'), 'last_updated': r.get('last_updated').isoformat() if isinstance(r.get('last_updated'), datetime) else str(r.get('last_updated'))} for r in recently_updated]}
        except Exception as e:
