...
def doctor() -> None:
    ...
    config_status_display = {'valid': '[green]✓ Valid[/green]', 'error': '[red]✗ Error[/red]', 'not_found': '[yellow]⚠ Not Found[/yellow]'}[config_status]
    files_table.add_row('Config', config_status_display, str(config_path) if config_path else '-')
    secrets_status_display = {'valid': '[green]✓ Valid[/green]', 'error': '[red]✗ Error[/red]', 'not_found': '[yellow]⚠ Not Found[/yellow]'}[secrets_status]
    files_table.add_row('Secrets', secrets_status_display, str(secrets_path) if secrets_path else '-')
    if config_error:
    ...
    cmd_requirements = {'npx': 'NPM-based MCP servers', 'uvx': 'Python MCP servers (fast)', 'uv': 'Python package management', 'python': 'Python scripts', 'python3': 'Python 3 scripts', 'git': 'Version control', 'docker': 'Containerized servers'}
    ...
    suggestions = _generate_suggestions(config_status, secrets_status, providers, servers, commands, settings)
    if suggestions:
...
def _check_provider_keys(settings: Settings, secrets: dict) -> dict:
    ...
    providers = {'openai': {'env': 'OPENAI_API_KEY', 'configured': False, 'source': None}, 'anthropic': {'env': 'ANTHROPIC_API_KEY', 'configured': False, 'source': None}, 'google': {'env': 'GOOGLE_API_KEY', 'configured': False, 'source': None}, 'azure': {'env': 'AZURE_API_KEY', 'configured': False, 'source': None}, 'bedrock': {'env': 'AWS_ACCESS_KEY_ID', 'configured': False, 'source': None}}
    for name, info in providers.items():
...
def _check_command_availability() -> dict:
    ...
    commands = {'npx': shutil.which('npx') is not None, 'uvx': shutil.which('uvx') is not None, 'uv': shutil.which('uv') is not None, 'python': shutil.which('python') is not None, 'python3': shutil.which('python3') is not None, 'git': shutil.which('git') is not None, 'docker': shutil.which('docker') is not None}
    return commands
...
def _generate_suggestions(config_status: str, secrets_status: str, providers: dict, servers: dict, commands: dict, settings: Settings) -> List[str]:
    """Generate actionable suggestions based on diagnostics."""
    suggestions = []
    if config_status == 'not_found':
        suggestions.append('[yellow]No config file found.[/yellow] Run [cyan]mcp-agent init[/cyan] to create one.')
    elif config_status == 'error':
        suggestions.append('[red]Config file has syntax errors.[/red] Run [cyan]mcp-agent config edit[/cyan] to fix.')
    if secrets_status == 'not_found':
        suggestions.append('[yellow]No secrets file found.[/yellow] Run [cyan]mcp-agent keys set <provider> <key>[/cyan] or create mcp_agent.secrets.yaml')
    elif secrets_status == 'error':
        suggestions.append('[red]Secrets file has syntax errors.[/red] Check YAML syntax in mcp_agent.secrets.yaml')
    no_keys = [p for p, info in providers.items() if not info['configured']]
    if no_keys:
        suggestions.append(f'[yellow]Missing API keys for: {', '.join(no_keys)}[/yellow]\n  Set with: [cyan]mcp-agent keys set <provider> <key>[/cyan]\n  Or export: {', '.join([providers[p]['env'] for p in no_keys])}')
    if not commands['npx'] and any((s.command == 'npx' for s in (servers.values() if isinstance(servers, dict) else servers))):
        suggestions.append('[yellow]npx not found but required by servers.[/yellow] Install Node.js from https://nodejs.org')
    if not commands['uvx'] and (not commands['uv']):
        suggestions.append('[dim]Consider installing uv for Python package management: https://github.com/astral-sh/uv[/dim]')
    if settings.logger and settings.logger.type == 'file' and (not getattr(settings.logger, 'path', None)):
        suggestions.append("[yellow]Logger type 'file' requires 'path' setting.[/yellow] Add logger.path to config.")
    if settings.otel and settings.otel.enabled:
        try:
            for e in settings.otel.exporters or []:
                if getattr(e, 'type', None) == 'otlp' and (not getattr(e, 'endpoint', None)):
                    suggestions.append('[yellow]OTLP exporter enabled without endpoint.[/yellow] Add endpoint to otel.exporters config.')
        except Exception:
            pass
    return suggestions
