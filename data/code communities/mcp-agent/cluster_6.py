...
def test_v1_string_exporters_with_legacy_fields():
    ...
    settings = OpenTelemetrySettings(enabled=True, exporters=['console', 'file', 'otlp'], path='/tmp/trace.jsonl', path_settings={'path_pattern': 'traces/trace-{unique_id}.jsonl', 'unique_id': 'timestamp'}, otlp_settings={'endpoint': 'http://collector:4318/v1/traces', 'headers': {'Authorization': 'Bearer token'}})
    assert len(settings.exporters) == 3
    _assert_console_exporter(settings.exporters[0])
    _assert_file_exporter(settings.exporters[1], path='/tmp/trace.jsonl', path_pattern='traces/trace-{unique_id}.jsonl')
    _assert_otlp_exporter(settings.exporters[2], endpoint='http://collector:4318/v1/traces', headers={'Authorization': 'Bearer token'})
...
def test_v1_file_exporter_with_base_model_path_settings():
    ...
    settings = OpenTelemetrySettings(enabled=True, exporters=['file'], path_settings=TracePathSettings(path_pattern='trace-{unique_id}.jsonl', unique_id='session_id'))
...
def test_v1_otlp_exporter_with_base_model():
    ...
    settings = OpenTelemetrySettings(enabled=True, exporters=['otlp'], otlp_settings=TraceOTLPSettings(endpoint='http://collector:4318/v1/traces', headers={'X-Custom-Header': 'value'}))
    assert len(settings.exporters) == 1
    _assert_otlp_exporter(settings.exporters[0], endpoint='http://collector:4318/v1/traces', headers={'X-Custom-Header': 'value'})
...
def test_v1_string_exporters_without_legacy_fields():
    ...
    settings = OpenTelemetrySettings(enabled=True, exporters=['console', 'file', 'otlp'])
    assert len(settings.exporters) == 3
    _assert_console_exporter(settings.exporters[0])
    _assert_file_exporter(settings.exporters[1])
    _assert_otlp_exporter(settings.exporters[2])

def test_v2_type_discriminated_union():
    """Test exporters with 'type' discriminator field."""
    settings = OpenTelemetrySettings(enabled=True, exporters=[{'type': 'console'}, {'type': 'file', 'path': '/var/log/traces.jsonl'}, {'type': 'otlp', 'endpoint': 'http://collector:4318/v1/traces'}])
    assert len(settings.exporters) == 3
    _assert_console_exporter(settings.exporters[0])
...
def test_v2_multiple_otlp_exporters():
    ...
    settings = OpenTelemetrySettings(enabled=True, exporters=[{'type': 'otlp', 'endpoint': 'http://collector1:4318'}, {'type': 'otlp', 'endpoint': 'http://collector2:4318', 'headers': {'X-API-Key': 'secret'}}])
    assert len(settings.exporters) == 2
    _assert_otlp_exporter(settings.exporters[0], endpoint='http://collector1:4318')
    _assert_otlp_exporter(settings.exporters[1], endpoint='http://collector2:4318', headers={'X-API-Key': 'secret'})

def test_v2_file_exporter_with_path_settings():
    """Test type-discriminated file exporter with nested path_settings."""
    settings = OpenTelemetrySettings(enabled=True, exporters=[{'type': 'file', 'path': '/tmp/trace.jsonl', 'path_settings': {'path_pattern': 'logs/{unique_id}.jsonl', 'unique_id': 'timestamp', 'timestamp_format': '%Y%m%d'}}])
    assert len(settings.exporters) == 1
    file_exp = settings.exporters[0]
...
def test_v3_dict_key_discriminator():
    ...
    settings = OpenTelemetrySettings(enabled=True, exporters=[{'console': {}}, {'file': {'path': '/var/log/traces.jsonl'}}, {'otlp': {'endpoint': 'http://collector:4318/v1/traces'}}])
    assert len(settings.exporters) == 3
    _assert_console_exporter(settings.exporters[0])
    _assert_file_exporter(settings.exporters[1], path='/var/log/traces.jsonl')
    _assert_otlp_exporter(settings.exporters[2], endpoint='http://collector:4318/v1/traces')
...
def test_v3_multiple_exporters_same_type():
    ...
    settings = OpenTelemetrySettings(enabled=True, exporters=[{'otlp': {'endpoint': 'http://primary-collector:4318'}}, {'otlp': {'endpoint': 'http://backup-collector:4318', 'headers': {'X-Region': 'us-west'}}}, {'otlp': {'endpoint': 'https://cloud-collector.example.com:4318'}}])
    assert len(settings.exporters) == 3
    _assert_otlp_exporter(settings.exporters[0], endpoint='http://primary-collector:4318')
    _assert_otlp_exporter(settings.exporters[1], endpoint='http://backup-collector:4318', headers={'X-Region': 'us-west'})
...
def test_v3_file_exporter_with_advanced_path_settings():
    ...
    settings = OpenTelemetrySettings(enabled=True, exporters=[{'file': {'path': 'a/b/c/d', 'path_settings': {'path_pattern': 'logs/mcp-agent-{unique_id}.jsonl', 'unique_id': 'timestamp', 'timestamp_format': '%Y%m%d_%H%M%S'}}}])
    assert len(settings.exporters) == 1
    file_exp = settings.exporters[0]
...
def test_v3_console_exporter_empty_dict():
    ...
    settings = OpenTelemetrySettings(enabled=True, exporters=[{'console': {}}])
    assert len(settings.exporters) == 1
    _assert_console_exporter(settings.exporters[0])
...
def test_mixed_v1_and_v3_string_and_dict():
    ...
    settings = OpenTelemetrySettings(enabled=True, exporters=['console', {'file': {'path': '/tmp/trace.jsonl'}}])
    assert len(settings.exporters) == 2
    _assert_console_exporter(settings.exporters[0])
...
def test_v2_to_v3_conversion():
    ...
    v2_settings = OpenTelemetrySettings(enabled=True, exporters=[{'type': 'console'}, {'type': 'otlp', 'endpoint': 'http://collector:4318', 'headers': {'Auth': 'Bearer xyz'}}])
    assert len(v2_settings.exporters) == 2
    _assert_console_exporter(v2_settings.exporters[0])
    _assert_otlp_exporter(v2_settings.exporters[1], endpoint='http://collector:4318', headers={'Auth': 'Bearer xyz'})

def test_v1_full_config_via_settings():
    """Test string exporter config loaded via full Settings model."""
    settings = Settings(otel={'enabled': True, 'exporters': ['console', 'otlp'], 'otlp_settings': {'endpoint': 'http://collector:4318/v1/traces'}})
...
def test_v2_full_config_via_settings():
    ...
    settings = Settings(otel={'enabled': True, 'exporters': [{'type': 'console'}, {'type': 'file', 'path': '/tmp/trace.jsonl'}], 'service_name': 'TestApp'})
    assert settings.otel.enabled is True
...
def test_v3_full_config_via_settings():
    ...
    settings = Settings(otel={'enabled': True, 'exporters': [{'console': {}}, {'otlp': {'endpoint': 'https://collector.example.com:4318'}}], 'service_name': 'V3App', 'sample_rate': 0.5})
    assert settings.otel.enabled is True
...
def deep_merge(base: dict, update: dict, path: tuple=()) -> dict:
    ...
    for key, value in update.items():
        ...
        if key in merged and isinstance(merged[key], dict) and isinstance(value, dict):
            merged[key] = deep_merge(merged[key], value, current_path)
        elif key in merged and isinstance(merged[key], list) and isinstance(value, list) and (current_path == ('otel', 'exporters')):
...
def test_merge_otel_exporters_from_config_and_secrets():
    ...
    config_data = {'otel': {'exporters': [{'otlp': {'endpoint': 'https://us.cloud.langfuse.com/api/public/otel/v1/traces', 'headers': {'Authorization': 'Basic AUTH_STRING'}}}]}}
    secrets_data = {'otel': {'enabled': True, 'exporters': [{'otlp': {'endpoint': 'https://some-other-otel-exporter'}}]}}
    ...
    assert settings.otel.enabled is True
    assert len(settings.otel.exporters) == 2
    _assert_otlp_exporter(settings.otel.exporters[0], endpoint='https://us.cloud.langfuse.com/api/public/otel/v1/traces', headers={'Authorization': 'Basic AUTH_STRING'})
    _assert_otlp_exporter(settings.otel.exporters[1], endpoint='https://some-other-otel-exporter')
...
def test_merge_non_otel_lists_are_replaced_not_concatenated():
    ...
    config_data = {'mcp': {'servers': {'fetch': {'command': 'uvx', 'args': ['mcp-server-fetch']}}}}
    secrets_data = {'mcp': {'servers': {'filesystem': {'command': 'npx', 'args': ['-y', '@modelcontextprotocol/server-filesystem']}}}}
    merged = deep_merge(config_data, secrets_data)
