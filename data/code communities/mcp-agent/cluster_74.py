...
def anthropic_content_to_mcp_content(content: str | Iterable[TextBlockParam | ImageBlockParam | ToolUseBlockParam | ToolResultBlockParam | DocumentBlockParam | ContentBlock]) -> List[TextContent | ImageContent | EmbeddedResource]:
    mcp_content = []
    if isinstance(content, str):
        ...
        for block in content:
            ...
            if block_type == 'text':
                ...
                mcp_content.append(TextContent(type='text', text=to_string(block)))
...
def mcp_content_to_anthropic_content(content: TextContent | ImageContent | EmbeddedResource, for_message_param: bool=False) -> ContentBlock | MessageParamContent:
    """
    ...
    if for_message_param:
        if isinstance(content, TextContent):
            ...
            return ImageBlockParam(type='image', source=Base64ImageSourceParam(type='base64', data=content.data, media_type=content.mimeType))
        elif isinstance(content, EmbeddedResource):
            if isinstance(content.resource, TextResourceContents):
                return TextBlockParam(type='text', text=content.resource.text)
            else:
                if content.resource.mimeType == 'text/plain':
                    source = PlainTextSourceParam(type='text', data=content.resource.blob, mimeType=content.resource.mimeType)
                elif content.resource.mimeType == 'application/pdf':
                    source = Base64PDFSourceParam(type='base64', data=content.resource.blob, mimeType=content.resource.mimeType)
                else:
                    return TextBlockParam(type='text', text=f'{content.resource.mimeType}:{content.resource.blob}')
        ...
        if isinstance(content.resource, TextResourceContents):
            ...
            return TextBlock(type='text', text=f'{content.resource.mimeType}:{content.resource.blob}')
    else:
