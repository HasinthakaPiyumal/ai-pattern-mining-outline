...
def _find_files() -> dict[str, Optional[Path]]:
    return {'config': Settings.find_config(), 'secrets': Settings.find_secrets()}

def _get_system_info() -> dict:
    return {'platform': platform.platform(), 'python': sys.version.split(' ')[0], 'python_path': sys.executable}
...
def _config_summary(config_path: Optional[Path]) -> dict:
    ...
    try:
        ...
        for name, cfg in servers.items():
            info = {'name': name, 'transport': (cfg or {}).get('transport', 'stdio').upper(), 'command': (cfg or {}).get('command', ''), 'url': (cfg or {}).get('url', '')}
