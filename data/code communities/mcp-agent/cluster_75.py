...
def format_step_result(step_result: StepResult) -> str:
    ...
    tasks_str = '\n'.join((f'  - {format_task_result(task)}' for task in step_result.task_results))
    return STEP_RESULT_TEMPLATE.format(step_description=step_result.step.description, step_result=step_result.result, tasks_str=tasks_str)
...
def format_plan_result(plan_result: PlanResult) -> str:
    ...
    steps_str = '\n\n'.join((f'{i + 1}:\n{format_step_result(step)}' for i, step in enumerate(plan_result.step_results))) if plan_result.step_results else 'No steps executed yet'
    return PLAN_RESULT_TEMPLATE.format(plan_objective=plan_result.objective, steps_str=steps_str, plan_status='Complete' if plan_result.is_complete else 'In Progress', plan_result=plan_result.result if plan_result.is_complete else 'In Progress')
...
class TestOrchestratorModels:
    ...
    def test_server_task_creation(self):
        ...
        server_task = ServerTask(description='Test server task', servers=['server1', 'server2'])
    ...
    def test_step_creation(self):
        ...
        tasks = [AgentTask(description='Task 1', agent='agent1'), AgentTask(description='Task 2', agent='agent2')]
    ...
    def test_plan_creation(self):
        ...
        step = Step(description='Test step', tasks=[AgentTask(description='Test task', agent='test_agent')])
    ...
    def test_task_with_result_creation(self):
        ...
        task_result = TaskWithResult(description='Test task', agent='test_agent', result='Task completed')
    ...
    def test_step_result_creation(self):
        ...
        step = Step(description='Test step', tasks=[AgentTask(description='Test task', agent='test_agent')])
        task_result = TaskWithResult(description='Test task', agent='test_agent', result='Task completed')
        step_result = StepResult(step=step, task_results=[task_result], result='Step completed')
    ...
    def test_plan_result_creation(self):
        ...
        step = Step(description='Test step', tasks=[AgentTask(description='Test task', agent='test_agent')])
        step_result = StepResult(step=step, task_results=[TaskWithResult(description='Test task', agent='test_agent', result='Task completed')], result='Step completed')
        plan_result = PlanResult(objective='Test objective', plan=Plan(steps=[step], is_complete=False), step_results=[step_result], is_complete=False)
    ...
    def test_next_step_creation(self):
        ...
        next_step = NextStep(description='Next step', tasks=[AgentTask(description='Test task', agent='test_agent')], is_complete=False)
    ...
    def test_format_task_result(self):
        ...
        task_result = TaskWithResult(description='Test task', agent='test_agent', result='Task result')
    ...
    def test_format_step_result(self):
        ...
        step = Step(description='Test step', tasks=[AgentTask(description='Test task', agent='test_agent')])
        step_result = StepResult(step=step, task_results=[TaskWithResult(description='Test task', agent='test_agent', result='Task result')], result='Step result')
    ...
    def test_format_plan_result(self):
        ...
        step = Step(description='Test step', tasks=[AgentTask(description='Test task', agent='test_agent')])
        step_result = StepResult(step=step, task_results=[TaskWithResult(description='Test task', agent='test_agent', result='Task result')], result='Step result')
        plan_result = PlanResult(objective='Test objective', plan=Plan(steps=[step], is_complete=False), step_results=[step_result], is_complete=False, result=None)
    ...
    def test_format_plan_result_complete(self):
        ...
        step = Step(description='Test step', tasks=[AgentTask(description='Test task', agent='test_agent')])
        step_result = StepResult(step=step, task_results=[TaskWithResult(description='Test task', agent='test_agent', result='Task result')], result='Step result')
        plan_result = PlanResult(objective='Test objective', plan=Plan(steps=[step], is_complete=True), step_results=[step_result], is_complete=True, result='Plan completed')
        formatted = format_plan_result(plan_result)
        assert 'Test objective' in formatted
        assert 'Test step' in formatted
