...
def _resolve_log_file(explicit: Path | None) -> Path | None:
    ...
    if cfg.logger and cfg.logger.path:
        ...
        if p.exists():
            return p
    try:
        if cfg.logger and cfg.logger.path_settings and cfg.logger.path_settings.path_pattern:
...
def logs(file: Path=typer.Option(Path(''), '--file'), follow: bool=typer.Option(False, '--follow'), limit: int=typer.Option(200, '--limit'), grep: str | None=typer.Option(None, '--grep'), desc: bool=typer.Option(True, '--desc/--asc'), since: str | None=typer.Option(None, '--since', help='Relative window (e.g., 1h, 30m, 7d)'), from_time: str | None=typer.Option(None, '--from', help='RFC3339 start time'), to_time: str | None=typer.Option(None, '--to', help='RFC3339 end time'), orderby: str=typer.Option('time', '--orderby', help='Sort by: time|severity|tokens')) -> None:
    ...
    try:
        ...
        filtered: List[Tuple[Dict[str, Any] | None, str, datetime | None, int, int]] = []
        for obj, ln in entries:
            ts = None
            lvl = 0
            toks = 0
            if isinstance(obj, dict):
                ts_raw = obj.get('timestamp') or (obj.get('data', {}) or {}).get('timestamp')
...
def _filter_time(entry_ts: datetime | None, since_dt: datetime | None, from_dt: datetime | None, to_dt: datetime | None) -> bool:
    if entry_ts is None:
        return True
