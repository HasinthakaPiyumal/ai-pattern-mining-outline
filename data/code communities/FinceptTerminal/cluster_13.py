...
def translate_spanish_text(text: str) -> Dict[str, Any]:
    ...
    return {'original': original_text, 'translated': translated_text, 'found_translations': found_translations, 'translation_confidence': 'high' if found_translations else 'none'}
...
def _make_request(endpoint: str, params: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
    ...
    try:
        ...
        headers = {'Content-Type': 'application/json', 'User-Agent': 'Fincept-Terminal/1.0'}
        if params is None:
            params = {}
        params['_format'] = 'json'
        ...
        return {'data': result_data, 'metadata': {'source': 'Spain Open Data (datos.gob.es)', 'endpoint': endpoint, 'timestamp': datetime.now().isoformat(), 'total_items': len(result_data.get('items', [])), 'translation_enabled': True}, 'error': None}
    ...
    if result.get('error') is None and result.get('data', {}).get('items'):
        ...
        for item in items:
            if isinstance(item, str) and item.startswith('http'):
                ...
                enhanced_item = {'uri': item, 'extracted_id': extracted_id}
...
def get_catalogue_details(publisher_id: str) -> Dict[str, Any]:
    ...
    for catalogue in catalogues_result.get('data', {}).get('items', []):
        if catalogue.get('id') == clean_id or catalogue.get('extracted_id') == publisher_id:
            return {'data': catalogue, 'metadata': {'source': 'Spain Open Data (datos.gob.es)', 'catalogue_id': publisher_id, 'timestamp': datetime.now().isoformat()}, 'error': None}
...
def list_datasets(publisher_id: Optional[str]=None, page: int=1, page_size: int=20) -> Dict[str, Any]:
    ...
    params = {'_page': page, '_pageSize': page_size}
    if publisher_id:
...
def list_dataset_resources(dataset_id: str) -> Dict[str, Any]:
    ...
    return {'data': distributions, 'metadata': {'source': 'Spain Open Data (datos.gob.es)', 'dataset_id': dataset_id, 'dataset_title': dataset.get('title', 'Unknown Dataset'), 'resource_count': len(distributions), 'timestamp': datetime.now().isoformat(), 'description': 'List of data files available for download'}, 'error': None}
...
def get_resource_download_info(dataset_id: str, resource_index: Optional[int]=None, resource_title: Optional[str]=None) -> Dict[str, Any]:
    ...
    if isinstance(title, list) and title:
        ...
        title_str = str(title)
    download_info = {'resource_info': target_resource, 'download_url': target_resource.get('accessURL'), 'file_format': file_format, 'download_available': bool(target_resource.get('accessURL')), 'metadata': {'source': 'Spain Open Data (datos.gob.es)', 'dataset_id': dataset_id, 'resource_title': title_str, 'timestamp': datetime.now().isoformat(), 'description': 'Download information for dataset resource'}}
    return {'data': download_info, 'metadata': {'source': 'Spain Open Data (datos.gob.es)', 'endpoint': 'resource_download_info', 'timestamp': datetime.now().isoformat(), 'description': 'Download information for dataset resource'}, 'error': None}
...
def search_datasets(query: str, publisher_id: Optional[str]=None, page: int=1, page_size: int=20) -> Dict[str, Any]:
    ...
    for dataset in datasets:
        ...
        if query_lower in title or query_lower in description or query_lower in title_trans or (query_lower in desc_trans):
            filtered_datasets.append(dataset)
    return {'data': {'items': filtered_datasets, 'total_found': len(filtered_datasets), 'search_query': query, 'publisher_filter': publisher_id}, 'metadata': {'source': 'Spain Open Data (datos.gob.es)', 'search_timestamp': datetime.now().isoformat(), 'results_count': len(filtered_datasets), 'description': f"Search results for '{query}'" + (f' from publisher: {publisher_id}' if publisher_id else '')}, 'error': None}

def main():
    """CLI interface"""
    if len(sys.argv) < 2:
        print(json.dumps({'error': 'Usage: spain_data.py <command> [args]', 'available_commands': {'catalogues': 'List all catalogues (publishers)', 'catalogue-details <publisher_id>': 'Get details for a specific catalogue', 'datasets [publisher_id] [page] [page_size]': 'List datasets, optionally filtered by publisher', 'dataset-details <dataset_id>': 'Get details for a specific dataset', 'resources <dataset_id>': 'List all resources for a dataset', 'resource-info <dataset_id> [index|title]': 'Get download info for a specific resource', 'search <query> [publisher_id]': 'Search datasets by query'}, 'examples': ['spain_data.py catalogues', 'spain_data.py catalogue-details /sector-publico/org/L01280040', 'spain_data.py datasets /sector-publico/org/L01280040', 'spain_data.py dataset-details callejero-alcorcon', 'spain_data.py resources callejero-alcorcon', 'spain_data.py resource-info callejero-alcorcon 0', 'spain_data.py search "callejero" /sector-publico/org/L01280040']}, indent=2))
        sys.exit(1)
    command = sys.argv[1]
    try:
