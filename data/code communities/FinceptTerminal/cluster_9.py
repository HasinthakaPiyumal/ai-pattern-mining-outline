...
def get_series(series_id: str, start_date: Optional[str]=None, end_date: Optional[str]=None, frequency: Optional[str]=None, transform: Optional[str]=None) -> Dict:
    """
    ...
    try:
        ...
        return {'series_id': series_id, 'title': series_info.get('title', 'N/A'), 'units': series_info.get('units', 'N/A'), 'frequency': series_info.get('frequency', 'N/A'), 'seasonal_adjustment': series_info.get('seasonal_adjustment', 'N/A'), 'last_updated': series_info.get('last_updated', 'N/A'), 'observations': observations, 'observation_count': len(observations)}
...
def search_series(search_text: str, limit: int=10) -> Dict:
    ...
    try:
        ...
        for series in result.get('seriess', []):
            series_list.append({'id': series.get('id'), 'title': series.get('title'), 'frequency': series.get('frequency'), 'units': series.get('units'), 'seasonal_adjustment': series.get('seasonal_adjustment'), 'last_updated': series.get('last_updated'), 'popularity': series.get('popularity')})
        return {'search_text': search_text, 'count': len(series_list), 'series': series_list}
    except Exception as e:
...
def get_categories(category_id: int=0) -> Dict:
    ...
    try:
        ...
        for cat in result.get('categories', []):
            categories.append({'id': cat.get('id'), 'name': cat.get('name'), 'parent_id': cat.get('parent_id')})
        return {'category_id': category_id, 'categories': categories}
    except Exception as e:
...
def get_category_series(category_id: int, limit: int=50) -> Dict:
    ...
    try:
        ...
        series_list = []
        for series in result.get('seriess', []):
            series_list.append({'id': series.get('id'), 'title': series.get('title'), 'frequency': series.get('frequency'), 'units': series.get('units'), 'seasonal_adjustment': series.get('seasonal_adjustment'), 'last_updated': series.get('last_updated'), 'popularity': series.get('popularity', 0)})
        return {'category_id': category_id, 'count': len(series_list), 'series': series_list}
    except Exception as e:
        return {'error': str(e), 'category_id': category_id}
...
def get_release_dates(release_id: int, limit: int=10) -> Dict:
    ...
    try:
        ...
        for date in result.get('release_dates', []):
            dates.append({'release_id': date.get('release_id'), 'date': date.get('date')})
        return {'release_id': release_id, 'dates': dates}
    except Exception as e:
        return {'error': str(e), 'release_id': release_id}

def get_multiple_series(series_ids: List[str], start_date: Optional[str]=None, end_date: Optional[str]=None) -> List[Dict]:
...
def main():
    ...
    if command == 'series':
        ...
        for i in range(2, len(sys.argv)):
            if '-' in sys.argv[i] and len(sys.argv[i]) == 10:
