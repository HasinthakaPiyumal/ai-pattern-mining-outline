...
class AnalyticsTab(BaseTab):
    ...
    def load_real_data(self):
        ...
        safe_info(f'Loading data: {self.current_symbol}, {self.current_forex_pair}, {self.current_crypto}', module='AnalyticsTab')
        if not self.data_manager:
    ...
    def _safe_get_stock_data(self):
        ...
        try:
            ...
            safe_debug(f'Stock data success: {data.get('success', False)}, source: {data.get('source', 'unknown')}', module='AnalyticsTab')
            return data
        except Exception as e:
    ...
    def _safe_get_forex_data(self):
        ...
        try:
            ...
            safe_debug(f'Forex data success: {data.get('success', False)}, source: {data.get('source', 'unknown')}', module='AnalyticsTab')
            return data
        except Exception as e:
    ...
    def _safe_get_crypto_data(self):
        ...
        try:
            ...
            safe_debug(f'Crypto data success: {data.get('success', False)}, source: {data.get('source', 'unknown')}', module='AnalyticsTab')
            return data
        except Exception as e:
    ...
    def process_data_for_charts(self, stock_data, forex_data, crypto_data):
        ...
        if stock_data.get('success') and stock_data.get('data'):
            ...
            self.chart_data.update({'stock_timestamps': list(range(len(stock_chart_data.get('close', [])))), 'stock_prices': stock_chart_data.get('close', []), 'stock_volumes': stock_chart_data.get('volume', []), 'stock_opens': stock_chart_data.get('open', []), 'stock_highs': stock_chart_data.get('high', []), 'stock_lows': stock_chart_data.get('low', []), 'stock_source': stock_data.get('source', 'unknown')})
            safe_debug(f'Stock data processed: {len(stock_chart_data.get('close', []))} points', module='AnalyticsTab')
        else:
            self.chart_data.update({'stock_source': 'error', 'stock_error': stock_data.get('error', 'No data available')})
        ...
        if forex_data.get('success') and forex_data.get('data'):
            ...
            self.chart_data.update({'forex_timestamps': list(range(len(forex_chart_data.get('rates', [])))), 'forex_rates': forex_chart_data.get('rates', []), 'forex_source': forex_data.get('source', 'unknown')})
            safe_debug(f'Forex data processed: {len(forex_chart_data.get('rates', []))} points', module='AnalyticsTab')
        else:
            self.chart_data.update({'forex_source': 'error', 'forex_error': forex_data.get('error', 'No data available')})
        ...
        if crypto_data.get('success') and crypto_data.get('data'):
            ...
            self.chart_data.update({'crypto_timestamps': list(range(len(crypto_chart_data.get('prices', [])))), 'crypto_prices': crypto_chart_data.get('prices', []), 'crypto_source': crypto_data.get('source', 'unknown')})
            safe_debug(f'Crypto data processed: {len(crypto_chart_data.get('prices', []))} points', module='AnalyticsTab')
        else:
            self.chart_data.update({'crypto_source': 'error', 'crypto_error': crypto_data.get('error', 'No data available')})
    ...
    def _load_fallback_data(self):
        ...
        self.chart_data = {'stock_source': 'fallback', 'forex_source': 'fallback', 'crypto_source': 'fallback', 'stock_error': 'Data source unavailable', 'forex_error': 'Data source unavailable', 'crypto_error': 'Data source unavailable'}
    ...
    def create_control_panel(self):
        ...
        with dpg.group(horizontal=True):
            ...
            dpg.add_combo(['1d', '5d', '1mo'], default_value='5d', tag='analytics_time_range', callback=self.on_time_range_changed, width=100)
        dpg.add_spacer(height=10)
        with dpg.group(horizontal=True):
            dpg.add_text('Stock:')
            ...
            if 'stock_error' in data:
                ...
                try:
                    with dpg.plot(label='Stock Price', height=200, width=-1):
                        ...
                        dpg.add_line_series(data['stock_timestamps'], data['stock_prices'], label=f'{self.current_symbol}', parent='analytics_stock_y', tag='analytics_stock_series')
                except Exception as e:
            ...
            if 'forex_error' in data:
                ...
                try:
                    with dpg.plot(label='Forex Rate', height=200, width=-1):
                        ...
                        dpg.add_line_series(data['forex_timestamps'], data['forex_rates'], label=f'{self.current_forex_pair}', parent='analytics_forex_y', tag='analytics_forex_series')
                except Exception as e:
                    dpg.add_text(f'Chart error: {str(e)}', color=[255, 100, 100])
            ...
            if 'crypto_error' in data:
                ...
                try:
                    with dpg.plot(label='Crypto Price', height=200, width=-1):
                        ...
                        dpg.add_line_series(data['crypto_timestamps'], data['crypto_prices'], label=f'{self.current_crypto}', parent='analytics_crypto_y', tag='analytics_crypto_series')
                except Exception as e:
                    dpg.add_text(f'Chart error: {str(e)}', color=[255, 100, 100])
            ...
            if data.get('stock_volumes'):
                try:
                    with dpg.plot(label='Volume', height=200, width=-1):
                        ...
                        dpg.add_bar_series(data['stock_timestamps'], data['stock_volumes'], label='Volume', parent='analytics_volume_y')
                except Exception as e:
                    dpg.add_text(f'Chart error: {str(e)}', color=[255, 100, 100])
            else:
