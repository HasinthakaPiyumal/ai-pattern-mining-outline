...
def get_stats_list(search_word: Optional[str]=None, stats_code: Optional[str]=None, survey_years: Optional[str]=None, limit: int=100) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {'data': [], 'metadata': {}, 'error': f'Error fetching statistics list: {str(e)}'}
...
def _parse_stats_list(raw_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for stat in stats_list:
            enhanced_stat = {'id': stat.get('@id'), 'stat_name': stat.get('STAT_NAME'), 'gov_org': stat.get('GOV_ORG'), 'gov_org_name': stat.get('GOV_ORG_NAME'), 'statistics_name': stat.get('STATISTICS_NAME'), 'title': stat.get('TITLE'), 'cycle': stat.get('CYCLE'), 'survey_date': stat.get('SURVEY_DATE'), 'open_date': stat.get('OPEN_DATE'), 'small_area': stat.get('SMALL_AREA'), 'main_category': stat.get('MAIN_CATEGORY'), 'sub_category': stat.get('SUB_CATEGORY'), 'overview': stat.get('OVERVIEW'), 'collect_date': stat.get('COLLECT_DATE'), 'total_number': stat.get('TOTAL_NUMBER'), 'notes': stat.get('NOTES')}
...
def get_meta_info(stats_data_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {'data': {}, 'metadata': {}, 'error': f'Error fetching meta info: {str(e)}'}
...
def _parse_meta_info(raw_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        enhanced_meta = {'table_id': meta_info.get('@id'), 'table_name': meta_info.get('TABLE_NAME'), 'table_name_en': meta_info.get('TABLE_NAME_EN'), 'table_subtitle': meta_info.get('TABLE_SUB_TITLE'), 'table_subtitle_en': meta_info.get('TABLE_SUB_TITLE_EN'), 'gov_org': meta_info.get('GOV_ORG'), 'gov_org_name': meta_info.get('GOV_ORG_NAME'), 'statistics_name': meta_info.get('STATISTICS_NAME'), 'title': meta_info.get('TITLE'), 'title_en': meta_info.get('TITLE_EN'), 'cycle': meta_info.get('CYCLE'), 'survey_date': meta_info.get('SURVEY_DATE'), 'open_date': meta_info.get('OPEN_DATE'), 'small_area': meta_info.get('SMALL_AREA'), 'main_category': meta_info.get('MAIN_CATEGORY'), 'sub_category': meta_info.get('SUB_CATEGORY'), 'overview': meta_info.get('OVERVIEW'), 'overview_en': meta_info.get('OVERVIEW_EN'), 'collect_date': meta_info.get('COLLECT_DATE'), 'copyright': meta_info.get('COPYRIGHT'), 'source': meta_info.get('SOURCE'), 'source_en': meta_info.get('SOURCE_EN')}
        class_obj = meta_info.get('CLASS_INF', {}).get('CLASS_OBJ', [])
        if isinstance(class_obj, dict):
            class_obj = [class_obj]
        enhanced_meta['classifications'] = []
        for cls in class_obj:
            classification = {'id': cls.get('@id'), 'name': cls.get('@name'), 'name_en': cls.get('@nameEn'), 'class_type': cls.get('@type'), 'objects': []}
            class_objects = cls.get('CLASS', [])
            if isinstance(class_objects, dict):
                class_objects = [class_objects]
            for obj in class_objects:
                class_obj_data = {'code': obj.get('@code'), 'name': obj.get('@name'), 'name_en': obj.get('@nameEn'), 'level': obj.get('@level'), 'unit': obj.get('@unit')}
...
def get_stats_data(stats_data_id: str, cd_cat01: Optional[str]=None, cd_cat02: Optional[str]=None, cd_cat03: Optional[str]=None, cd_area: Optional[str]=None, cd_time: Optional[str]=None, start_position: Optional[int]=None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        result['data'] = parsed_data
        result['metadata']['stats_data_id'] = stats_data_id
        result['metadata']['filters'] = {'cd_cat01': cd_cat01, 'cd_cat02': cd_cat02, 'cd_cat03': cd_cat03, 'cd_area': cd_area, 'cd_time': cd_time, 'start_position': start_position}
        ...
        return {'data': {}, 'metadata': {}, 'error': f'Error fetching stats data: {str(e)}'}
...
def _parse_stats_data(raw_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        enhanced_data = {'table_id': stats_data.get('@id'), 'table_name': stats_data.get('TABLE_NAME'), 'gov_org': stats_data.get('GOV_ORG'), 'gov_org_name': stats_data.get('GOV_ORG_NAME'), 'statistics_name': stats_data.get('STATISTICS_NAME'), 'title': stats_data.get('TITLE'), 'created': stats_data.get('DATE'), 'data_values': [], 'classifications': []}
        data_values = stats_data.get('DATA_INF', {}).get('VALUE', [])
        if isinstance(data_values, dict):
            data_values = [data_values]
        for value in data_values:
            data_point = {'tab': value.get('@tab'), 'cat01': value.get('@cat01'), 'cat02': value.get('@cat02'), 'cat03': value.get('@cat03'), 'area': value.get('@area'), 'time': value.get('@time'), 'value': value.get('$'), 'unit': value.get('@unit')}
        ...
        for cls in class_inf:
            classification = {'id': cls.get('@id'), 'name': cls.get('@name'), 'name_en': cls.get('@nameEn'), 'type': cls.get('@type')}
