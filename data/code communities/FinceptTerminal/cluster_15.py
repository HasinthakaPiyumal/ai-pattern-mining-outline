        ...
        for series_key, series_data in series.items():
            ...
            for obs_key, obs_value in observations.items():
                if obs_value and len(obs_value) > 0:
                    ...
                    data_point = {'series_key': series_key, 'time_period': time_period, 'value': value, 'dimensions': dimensions, 'attributes': series_attributes}
...
def get_multiple_indicators(economy: str, indicators: List[str], start_period: Optional[str]=None, end_period: Optional[str]=None) -> Dict[str, Any]:
    ...
    """
    if not indicators:
        return {'data': [], 'metadata': {}, 'error': 'At least one indicator must be specified'}
...
def get_multiple_economies_data(indicator: str, economies: List[str], start_period: Optional[str]=None, end_period: Optional[str]=None) -> Dict[str, Any]:
    ...
    """
    if not economies:
        return {'data': [], 'metadata': {}, 'error': 'At least one economy must be specified'}
    ...
    if isinstance(dataflows_result['data'], dict) and 'dataflows' in dataflows_result['data']:
        ...
        for dataflow in all_dataflows:
            if 'name' in dataflow and keyword.lower() in str(dataflow['name']).lower():
                ...
                matching_dataflows.append(dataflow)
    return {'data': matching_dataflows, 'metadata': {'source': 'Asian Development Bank (ADB) - Key Indicators Database', 'search_keyword': keyword, 'total_matches': len(matching_dataflows), 'description': f'Dataflows matching keyword: {keyword}', 'last_updated': datetime.now().isoformat()}, 'error': None}
