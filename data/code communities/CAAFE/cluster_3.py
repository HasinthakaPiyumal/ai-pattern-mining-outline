...
def draw_stripplot(df, x, y, hue, xscale='linear', xbound=None, hue_order=None, xlabel=None, ylabel=None, y_labels=None, title=None, legend_title=None, legend_loc='best', legend_labels=None, colormap='colorblind', size=None):
    with sb.axes_style('whitegrid', rc={'grid.linestyle': 'dotted', 'font.family': 'serif', 'font.serif': 'Times New Roman'}), sb.plotting_context('paper'):
        ...
        sb.stripplot(data=df, x=x, y=y, hue=hue, hue_order=hue_order, palette=colormap, dodge=True, jitter=False, alpha=0.15, zorder=1, linewidth=0.2)
        sb.pointplot(data=df, x=x, y=y, hue=hue, hue_order=hue_order, palette=colormap, dodge=0.5, join=False, markers='d', scale=1.1, ci=None)
        ...
        axes.legend(handles, labels, title=legend_title or hue, handletextpad=0, columnspacing=1, loc=legend_loc, ncol=1, frameon=True)
        for i in range(0, 14):
