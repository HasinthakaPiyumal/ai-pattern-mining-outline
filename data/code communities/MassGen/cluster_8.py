...
class TestBackendCapabilitiesRegistry:
    ...
    def test_filesystem_support_values(self):
        ...
        for backend_type, caps in BACKEND_CAPABILITIES.items():
            assert caps.filesystem_support in valid_values, f"{backend_type}: filesystem_support '{caps.filesystem_support}' not in {valid_values}"
    ...
    def test_capability_strings_are_valid(self):
        ...
        valid_capabilities = {'web_search', 'code_execution', 'bash', 'multimodal', 'vision', 'mcp', 'filesystem_native', 'filesystem_mcp', 'reasoning', 'image_generation', 'image_understanding', 'audio_generation', 'audio_understanding', 'video_generation', 'video_understanding'}
        for backend_type, caps in BACKEND_CAPABILITIES.items():
            for cap in caps.supported_capabilities:
                assert cap in valid_capabilities, f"{backend_type}: unknown capability '{cap}'. Valid capabilities: {valid_capabilities}"
